...
## 2026-08-07 - 修复：保存班级结构报后端异常

- 类型：缺陷修复（阻断级）
- 现象：行政保存班级结构时提示后端异常，服务日志反复出现「集合 classes 存在重复主键 CLS-middle-7-01，拒绝持久化」，此后所有写操作连带失败（登录也一并异常）。
- **根因**：学期字段（`termId`）是后期引入的，**存量班级与教室没有这个字段**。而按学期替换的逻辑用 `termId === 当前学期` 匹配旧行，存量行永远匹配不上、永远删不掉；新写入的行带 `termId`，于是新旧并存产生重复主键，持久化被整体拒绝并回滚——保存班级数 8→4 后实际变成 12 个班。
- **修复**：在数据规范化阶段一次性把无 `termId` 的班级与教室归属到首个学期（幂等），使按学期的替换逻辑恢复正常，而不是在每处判断上打补丁。
- 验证：反复保存 8→5→8 个班均正确、无重复主键、持久化成功；新增回归测试（模拟无 `termId` 的存量数据，反复增减班级数并逐次校验主键唯一性），并确认该测试在移除修复后会失败。
- 附带说明：日志中的 `relation "app_auditLogs" does not exist` 为该表创建前的历史记录，当前表与数据（7046 行）均正常，无需处理。

## 2026-08-07 - 补测试盲区：多学期长周期推进测试

- 类型：测试加固
- 背景：上一条修复的两个缺陷之所以逃逸，是测试策略有两处结构性缺口——(1) **集成缝隙**：业务测试只跑内存域逻辑、持久层测试只测持久层，没有一条测试走完"写操作 → 落盘 → 重载"完整链路，跨学期主键冲突因此在单测全绿的情况下漏到生产；(2) **时间维度缺失**：所有测试用写死日期，从未模拟"当前时间越过学期结束日"。
- 新增 `tests/multi-term-lifecycle.test.js`（`npm run test:multi-term`），模拟**连续三个学期**的真实运转：
  - 每学期：建学期 → 复制配置 → 校验排课配置无跨学期串台 → 排 29 节课（含正课/晚自习/补课多课型，并混入 2 节未签到、1 节已取消）→ 月度工作量确认与两级审批 → 工资生成/发布/教师确认/财务复核/锁定。
  - 每个关键写操作后做**落盘校验**：逐集合检查主键唯一性（复刻持久层的重复主键保护）+ 序列化往返，确保"写得进内存也存得进库"。
  - 换届链路：切换当前学期 → 归档上一学期 → 校验历史工资仍在且保持锁定。
- 覆盖的真实边界（均为测试过程中实际触发并验证的行为）：
  - 存在未签到课次时**拒绝锁定工资**，处理异常课次后方可锁定；
  - 锁定为**幂等**操作（重复调用返回同一单据），但锁定后**拒绝重新生成**；
  - 三学期三张独立工资单，课次与工资单的学期归属零串台；
  - 归档学期只读，修改任课配置被拒；
  - **历史账不可被后续规则变更影响**：模拟次年调薪后，已锁定工资的金额快照原样保留，新月份才按新标准计算。
- 至此测试套件增至 11 组。

## 2026-08-07 - 修复：学期无法换届（新建学期失败 + 状态不随日期更新）

- 类型：缺陷修复（阻断级）
- 现象：学期已于 7/31 结束，界面仍显示"进行中"；新建下一学期时接口报"服务端错误"。
- **根因一（新建失败）**：复制学期配置时，`gradeCourseRules`、`teacherAssignments` 等表都重算了 ID，但**班级与教室直接沿用源学期 ID**，导致跨学期主键重复，持久化层的重复主键保护拒绝写入并回滚整次保存——新学期因此永远建不成。修复：班级与教室生成学期内唯一 ID（`原ID@学期后缀`），并同步重写引用它们的配置（班级默认教室、任课配置的按班指定老师、教室资源覆盖），保证引用完整性。
- **根因二（状态不更新）**：学期状态仅有 `planned/active/archived` 三值且全靠人工推进，没有任何按日期的判定，过了结束日期仍显示"进行中"。修复：新增按日期派生的 `datePhase`（upcoming/ongoing/ended）与 `needsRollover` 标记，界面显示为「**已结束待归档**」并提示"该学期已于 X 结束，请新建下一学期并切换为当前学期，完成结算后再归档本学期"。
  - 有意保留人工归档：归档会把课表、调课、工资写操作全部转为只读，到点自动执行会打断跨月结算，因此只做提示不自动切换。
- 验证：10 个测试套件全绿；新增学期换届测试（ID 跨学期唯一性、引用完整性、日期阶段判定）；真实数据端到端确认新学期创建成功并复制 108 个班级、126 间教室。

## 2026-08-07 - 角色显示名调整：教研所 / 行政管理

- 类型：文案
- 按学校口径调整两个角色的显示名称：`admin` 由「行政账号 / 教务行政」改为「**教研所**」，`system_admin` 由「系统管理员 / 总校管理员」改为「**行政管理**」。
- **角色标识（role key）保持不变**（仍为 `admin` / `system_admin`），因此无需数据迁移，既有权限判断、审批模板中的审批角色、历史审批单与审计记录全部不受影响。
- 覆盖范围：登录页快速登录按钮、侧栏账号卡、角色标签、种子账号、各处提示语、审批角色选项、角色权限文档与试运行手册。
- 存量库中的账号展示文本由启动时的幂等升级自动更正，无需手工改数据。
- 验证：10 个测试套件全绿；浏览器实测登录页按钮与两账号登录后的显示名。

## 2026-08-07 - 数据归属修正（前端）：页面按新归属重整

- 类型：前端 / 权限
- 验证：10 个测试套件全绿；浏览器逐页实测两侧页面
- **人事档案页**新增：职称下拉、学历下拉、兼岗任命区（15 项勾选 + 班级学生数/年级管辖班数），随档案保存一并提交并纳入字段级审计；新增**月度考核区**（按月查询与录入，支持等级或直接核定金额，学部负责人只能录本学部）。
- **财务工资档案页**移除：职称档、学历、校龄、兼岗勾选、年级班数、本月考核结果、兼岗发放方式——这些已归人事；试用期比例改为只读（由人事状态推导）。财务保留：考核档位、住房档、月度特殊奖扣。
- 财务侧新增**只读事实区**「以下由人事维护（财务不可修改）」，展示职称档、学历、校龄、兼岗津贴、本月考核及其对应金额——让财务看得到计算依据，但改不了依据本身。
- 更新 `docs/roles-and-permissions.md`：新增数据归属原则章节与权限矩阵行。

## 2026-08-07 - 数据归属修正：人事管事实、财务管金额

- 类型：架构修正（职责分离）
- 背景：职称、学历、兼岗任命这些**人事事实**此前被放在工资档案里由财务维护，等于财务改一下职称就直接改了基本工资档，职责分离形同虚设；"本月考核结果"也被塞进"学期工资档案"，语义错位且由不掌握教师日常表现的财务录入。
- **职称与学历归人事**：`employees` 新增职称档与学历字段，由人事维护并纳入字段级审计；财务改这些字段返回 403。薪资引擎改为从人事档案读取（`hrFacts`），只用于套标准，不再自行存储。存量数据自动从工资档案迁入，缺人事事实时回落到工资档案保证兼容。
- **兼岗任命归人事**：班主任、年级主任、教研组长等 17 项任命移到人事档案（`teacherRoles`），这是"担任什么职务"的人事事实，不是薪资参数。
- **校龄与试用期自动推导**：校龄由入职日期计算、试用期折算由人事状态推导，均不再手工维护，消除与人事档案不一致的可能。
- **月度考核独立成集合**：按「教师 + 月份」存储，由**学部负责人与人事**录入（财务只读，返回 403），支持等级系数浮动或直接核定金额（适配幼儿园等区间制岗位）；录入后自动使该教师该月未锁定工资单失效重算，已锁定的不动。
- 修复：考核金额为 `null`（未核定）时被 `Number(null)=0` 误判为"核定 0 元"，导致考核工资整项归零。现区分"未核定"与"核定 0 元"。
- 验证：10 个测试套件全绿；端到端实测——人事改职称为正高+博士后工资立即变为 3520+800，财务改职称被拒，学部负责人录入优秀考核后考核工资由 3430 浮动至 4116，改为不合格后降至 2058。

## 2026-07-09 - 请假代课闭环：审批通过直接更新课表

- 类型：业务闭环（排课 × 审批联动）
- 影响范围：`server/scheduling.js` 新增三个联动函数、`server/oa.js` 审批副作用机制、审批弹层课程安排界面
- 验证：10 个测试套件全绿；真实数据端到端验证——请假审批通过后课表老师确实变更、生成正式代课变更单、请假教师名下课次清零
- **闭环打通**：请假审批时系统自动列出申请人请假期间的每一节课，学部负责人逐节选择「安排代课」（下拉为该学科可代课教师，自动排除该时段已有课、人事状态不可排课、当月已锁薪的人）或「取消该课」；审批通过时复用既有调课引擎写入课表，代课教师课表随即出现该课，扫码签到与课时费自动跟随。
- **不绕过任何校验**：走 `createScheduleChangeRequest` + `approveScheduleChangeRequest`，已签到/已完成课次、有签到记录、薪资已锁定、教师冲突、教室冲突、归档学期等既有保护全部生效；不可变更的课次在界面上禁用并显示具体原因。
- **整单回滚**：批量安排中任一节失败则全部回滚，课表不会出现"改了一半"的状态。
- **修复既有缺陷**：代课资格原按班级级任课白名单判定，导致代课候选恒为 0（代课功能实际不可用）。现区分长期任课关系（班级白名单）与临时代课（学科任课池），仅换人时放宽，调整时间/教室仍按原口径。
- 详情页展示课表执行结果（哪节课由谁代、哪节课取消），申请人与后续审批人可见。

## 2026-07-09 - 审批流程可自定义 + 代课交接改由上级安排

- 类型：功能增强（后端 + 前端 + 测试）
- 验证：`npm run test:oa` 扩充至 30+ 组断言；9 个测试套件全绿；浏览器逐项实测
- **代课与工作交接由上级安排**：请假表单移除申请人填写的「工作交接人」，改为学部负责人在审批时必填「代课安排」与选填「其他工作交接」；调课同理，具体调到哪节、由谁代课由教务在审批时指定。安排内容随审批单留痕，申请人与后续审批人在流程时间线中可见。未填写则无法通过。
- **审批流程可 DIY（系统管理员）**：模板由代码常量改为库内可配置数据（幂等播种，已有模板不会被升级覆盖）。新增「审批流程设置」页，可视化展示每个审批的表单字段与审批链路（含各环节审批角色、会签/或签、需审批人填写的内容），支持：
  - 编辑流程：改审批环节顺序与级数（最多 8 级）、指定审批角色、切换会签/或签、增删表单字段与审批人填写项；
  - 新建自定义审批类型（如用章申请），创建后立即对可发起角色可见；
  - 启用/停用（停用后不可新发起，历史单据不受影响）、删除自定义类型（内置类型只可停用；有进行中单据时拒绝删除）。
- 校验：模板标识格式与唯一性、字段标识合法性与重复、选择类型必须配选项、审批角色必须有效、环节至少一个审批角色。
- 流程修改只影响新提交的申请，进行中的申请仍按原流程走完。

## 2026-07-09 - 审批中心：Lark 风格通用 OA 审批系统

- 类型：新模块（后端 + 前端 + 测试）
- 影响范围：新增 `server/oa.js`、`/api/oa/*` 路由、审批中心视图；新增 `tests/oa-flows.test.js`（`npm run test:oa`，22 组断言）
- 验证：10 个测试套件全绿；浏览器端到端走通老师发起请假 → 学部负责人审批 → 人事备案全链路
- **模板驱动**：审批类型由模板定义（表单字段 + 审批步骤 + 可发起角色），新增类型只加模板不改流转逻辑。内置七类：请假、加班、补卡、调课、班级学生人数确认、课时规则确认、薪酬总额预算确认，另有通用事项兜底。
- **学期初确认事项进审批**：学生人数（三级：学部核对→财务确认→总校审批）、课时规则（财务复核→总校审批）、薪酬预算（人事复核→总校审批，含发放/预留比例之和必须为 100% 的校验）——三项均在学期开始前通过审批确定。
- **Lark 式交互**：审批首页按分类的图标网格选择类型、动态表单发起、待我审批/我发起的/我处理过三个页签、详情弹层含表单内容与流程进度时间线（当前节点高亮）、通过/拒绝/撤回/催办操作、导航实时待办角标、移动端适配。
- **流转能力**：多级串行、或签（任一审批人通过即流转）、会签（每个角色都需通过）、拒绝即终止并通知申请人、申请人可撤回与催办、停留超 3 个工作日自动提醒（与人事流程同口径）。
- **权限**：发起按角色白名单，审批仅当前环节指定角色可操作，详情仅申请人/审批链角色/人事总校可见，普通教师查询范围自动收敛到本人相关。
- 接入 SSE：审批单变化实时刷新列表与角标。

## 2026-07-09 - 薪资引擎第二批：职称学历解绑、标准可配置、档案标签化

- 类型：核心逻辑 / 前端 / 数据迁移
- 影响范围：`server/payroll.js`、`server/storage.js`、教师工资档案编辑界面
- 验证：8 个测试套件全绿；浏览器端到端实测（改档案→存盘→重算金额逐项核对）
- **职称与学历彻底解绑**（学校澄清）：基本工资只按职称档（正高/高级/一级/二级/三级/未评级），不再出现"一级(博士)/二级(硕士)"绑定；学历补贴（硕士 500／博士 800）独立发放，两者可同时享受。存量数据旧键（firstOrDoctor 等）在前后端自动归一，不影响历史档案。
- **试用期按整体 80%**（学校澄清）：折算范围覆盖全部薪酬项（含课时费），低于最低工资标准自动补足并在工资单列示。
- **全部标准可随时调整**：基本工资档、学历补贴、住房档、考核标准与系数、校龄公式与封顶、各类津贴、最低工资标准均通过既有薪资配置接口（`PATCH /api/payroll/rules`，深合并）调整，无需改代码；新增测试逐项验证调整后即时生效。
- **方案版本化迁移**：`SCHEME_VERSION` 升为 `fuyuan-policy-2026-09`，存量库中的旧版方案在启动时自动升级到新制度标准（修复"改了代码但库里仍是旧标准"，实测考核工资由 1180 自动升至 1480）；同版本下学校自定义配置完整保留。
- **考核与骨干等标签走档案维护**（学校口径：线下评审）：工资档案新增学历、本月考核结果、年级管辖班数、兼岗津贴发放方式四项可编辑字段，考核等级直接驱动考核工资浮动；修复 `gradeClassCount` 被按布尔值归一化导致津贴清零的缺陷。

## 2026-07-09 - 薪资引擎对齐 0726 薪酬制度（第一批）

- 类型：核心逻辑 / 制度对齐
- 影响范围：`server/payroll.js` 教师薪资计算全链路；新增 `tests/salary-policy-2026.test.js`（`npm run test:salary-policy`）
- 验证：新测试 + phase1/hr/hr-flows/cross-grade/term-lifecycle 全套件通过，零回归
- 参数对齐：基本工资档（高级 3320、一级 3120、二级 2820）、高中考核工资 1480、专任教师住房补贴统一 2100（去掉制度中不存在的骨干 2300 档）、高中非统考科目系数 0.9、初中生物/地理/美术/音乐/心理健康系数由 1.2 修正为 1、高中补课费 100（毕业年级 120）、停用制度未规定的超课时加价档。
- 新能力：
  - 校龄津贴公式化并按岗位分四套（教师分段封顶 500／生活教师 ×50 封顶 300／司机、食堂 ×20 封顶 500），修正原离散表第 6 年多发 50 元的偏差；
  - 考核工资由固定档改为「标准额 × 月度考核系数」（优秀 1.2／良好 1.1／合格 1／基本合格 0.8／不合格 0.6），支持直接指定金额以适配幼儿园等区间制岗位；未录入考核时行为与改造前一致；
  - 试用期按 80% 计发（作用范围可配置 all/fixed），低于深圳市最低工资标准自动补足并列示；
  - 兼岗政策区分「授课教师叠加」与「行政管理人员择高」（择高时列出未计项）；
  - 年级长津贴改为按管辖班数动态计算（初高中 班数×300+300），班主任补齐月固定额（高中 +500、小学 +100），初中教研组长 3000 归位，小学备课组长统考年级 1000。
- 待学校确认的配置项（已参数化，确认后改配置无需改代码）：学历补贴是否叠加（默认 0）、试用期折算范围、最低工资标准值、考核系数梯度。

## 2026-07-09 - 项目方案书（对外提案版）

- 类型：文档
- v2.2（同日）：填入报价（总额 875,000 元含税：设计 5 万/开发 68.5 万/测试部署 3.5 万/云资源首年 2.25 万/风险利润 0.75 万/税 7.5 万，分项合计已校验）；全文小程序改为移动端网页；新增报价边界节（不含微信支付）
- v2.1（同日）：按投标口径隐去全部项目进展状态（删除已完成/已交付/提前交付等表述与状态列，仓库路径改为附件引用），演示系统仅作为能力证明呈现
- v2.0（同日）：按学校要求升级为完整三阶段方案——三阶段 3A~3E 的功能范围、业务流程（全员打卡/岗位工资引擎/OA 四流程/扣款申诉/津贴报表）、数据模型、页面原型说明、逐批验收标准全部展开至与一二阶段同等深度；澄清清单扩至 Q1-Q15；四阶段改为可选包。
- 新增 `docs/项目方案书-富源学校管理系统.md`：15 章对外提案/评审文档（需求理解与范围、澄清清单 Q1-Q14、业务流程与规则、技术架构与实测性能、现有系统演示脚本、四阶段实施计划、团队配置（简历细节留待补充）、可核验案例（以本系统+仓库+测试+压测为核验物）、测试试运行验收、数据迁移与接口、信息安全与个保、第三方费用清单（费用留白）、报价工作量（留白）、源码知产与退出方案、质保运维 SLA）。截图/GIF 以占位标注，待浏览器可用时统一录制插入。

## 2026-07-09 - 补齐第三阶段开发 TODO

- 类型：规划
- 新增 `PHASE3_TODO.md`：把三阶段 PRD 的 3A~3E 五批次落成工程任务清单（数据集合/域模块/路由/前端/测试/压测/灰度逐项），含入场条件（R1/R2/R6 决策项）、每批交付效果与验收红线（平行核算、证据链、金额可见性、范围蔓延控制）。至此四个阶段 TODO 全部就位（PHASE1~4）。

## 2026-07-09 - P0/P1 升级复审 + 第四阶段（体验终版打磨）规划

- 类型：审查 / 加固 / 规划
- 复审结论：P0/P1 四项集成点逐一核验通过（骨架屏 loading 分支、重试键指向函数、/api/teachers search 过滤、/api/hr/todos 返回结构、命令面板 result.items 形状、原生弹窗清零）。
- 加固：切换账号/退出登录时强制收起已打开的弹层与命令面板，避免残留到新账号界面（app.js loginAccount）。缓存版本 ux-p0p1-3。
- 新增 `docs/交互体验终版打磨-第四阶段PRD.md` + `PHASE4_TODO.md`：第三阶段（2027-01）收口后的纯体验阶段，6 个批次（反馈深度/移动端 PWA/动效抽屉/搜索快捷键/空态可访问性/实时协同扩展），反馈驱动、批次可中断、不动业务逻辑红线；明确不做清单（框架重写/拖拽调课/深色模式等）。

## 2026-07-08 - 交互体验升级（P0/P1：弹层、反馈、全局搜索、实时推送）

- 类型：前端 / 后端 / 交互
- 影响范围：全站交互体验；对标 Lark 的弹层体系、操作反馈、全局搜索与实时感
- 验证：npm run check + test:phase1/hr/hr-flows/postgres-store 通过；浏览器逐项验证（弹层居中/危险色、命令面板搜功能与人员、SSE 端到端推送）
- P0-A 通用弹层组件：promise-based openDialog/confirmDialog/promptDialog（多字段表单、必填校验、Esc/遮罩关闭、确认按钮内建 loading、移动端底部 sheet），替换全站 20 处原生 window.prompt/confirm（薪资解锁、批量发布/锁定、账号重置/停用、学期归档/删除、任课重分配、敏感查看、审批意见、节点改名、模板发版/应用等）。
- P0-B 操作反馈：列表加载改骨架屏（HR 人员/审批、财务教师列表），加载失败常驻错误态 + 重试按钮，写操作按钮 loading 态。
- P1-A 全局搜索：⌘K/Ctrl+K 或顶栏「搜索」入口唤起命令面板；聚合当前角色可见功能页 + 人员搜索（hr 系搜档案、财务搜教师），键盘上下选择、回车直达。
- P1-B 实时推送（SSE）：后端 /api/events 事件流，审批/通知写操作后广播；前端 EventSource 订阅，待办角标与通知即时更新、断线自动重连；单实例广播，多实例需接 Redis pub/sub（见部署手册运维章节）。
- 缓存版本 20260708-ux-p0p1-2。

## 2026-07-08 - 老师端信息架构与角色收敛

- 类型：前端 / 权限 / 文档
- 影响范围：老师端导航、账户中心、教师导入、admin 与 system_admin 职责边界
- 验证：`npm run check`、test:phase1/hr/hr-flows 通过；浏览器逐角色验证（老师导航 8 项、admin 3 项、sysadmin 上传按钮）
- 内容：
  - 老师端「总薪资」并入「工资确认」（原页面仅为摘要+跳转，与月度确认重复）；可计工作量/待处理/确认状态统计卡迁入工资确认页。
  - 「我的档案」改为账户中心：移除导航项，点击左下角账户卡进入（移动端头部显示紧凑账户入口）；补齐电话/紧急联系人/紧急联系电话的展示（修复只在表单可见的问题）；修改联系方式收进笔形编辑按钮，展开后只提交与当前值不同的字段。
  - 教师导入支持上传 CSV 文件（FileReader 读入、去 BOM、自动预览校验、5MB 上限），粘贴方式保留。
  - 角色收敛：「人员列表」由 admin+system_admin 收归 system_admin（前端导航+接口同步）；新增 `docs/roles-and-permissions.md` 七角色职责矩阵、收敛决策与不合并原因说明。
  - 缓存版本 `lark-refresh-11`。
...
- 提交：见本次 commit
- 内容：
  - 新增 `docs/deployment-and-acceptance-guide.md`：一份文档覆盖裸服务器首次部署 → 一阶段验收 → 二阶段数据迁移窗口（json→dual→postgres 七步）→ 二阶段 UAT → 回滚预案（代码/数据层）→ 日常运维 Runbook → 验收签字表。含环境变量总表（9 项逐项说明）与 /api/health 逐字段自检口径。
  - README、system-update-process、phase2-trial-run-guide 挂接该手册。
