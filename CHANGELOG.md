...
## 2026-09-11 - 学校 UAT 发布候选版

- 类型：功能收口、真实数据试装、部署加固
- 发布标签：`uat-2026-09-11`
- 完成真实教师名册及学校管理账号接管、分学部与分年级段排课权限、小学部真实任课关系、生活教师排班与安全部跟车路线、分学部考勤上传模板等本轮试用内容。
- 部署强制使用 PostgreSQL：生产环境若未设置 `DB_DRIVER=postgres` 将拒绝启动，避免正式数据误写入本地 JSON 文件。
- 数据迁移包与代码仓库分离：真实名册、工资和本地数据库不进入 GitHub；通过受控备份包迁移。
- 数据库备份与恢复现已同时覆盖审批证明、考勤表等上传附件。
- 新增学校测试环境部署交接清单，并补充 UAT 安装、数据恢复、验证与回滚步骤。

## 2026-08-07 - 修复：新建学期按钮仍失败（任课配置重复行）

- 类型：缺陷修复（阻断级）
- 现象：上一轮修复后，界面点击「新建学期」仍报服务端错误，详情为「集合 teacherAssignments 存在重复主键 TA-…-PRIMARY-1-CHINESE」。
- **根因**：任课配置的 ID 规则历史上改过（早期 `学部-年级-学科`，后改为带学期前缀），升级过程中同一份配置在库中留下了**新旧两行**。二者业务作用域相同，复制到新学期时被重算成同一个 ID，触发重复主键、整次持久化失败。
- **修复**：规范化阶段按 (学期, 学部, 年级, 学科) 去重，保留按班指定最完整、更新时间最新的一行。真实库据此清理 6 条冗余（126 → 120 行）。
- 验证：浏览器实际点击「新建学期」→ 接口 200、提示"新学期已创建"、学期列表正确增至 2 个；新增回归测试（构造新旧双行后建学期），并通过"停用去重 → 测试失败"确认其有效性。
- **过程反思**：上一轮审计用 API 直调验证通过，却漏了这个缺陷——因为脏数据只存在于真实库，而审计脚本用的是新种子构造的模拟老库。此后涉及存量数据的问题，一律以真实库 + 界面实际操作为准，不以 API 直调为准。

## 2026-08-07 - 存量数据兼容性审计

- 类型：审计 + 加固
- 背景：连续两个缺陷（学期换届失败、保存班级结构异常）同源——`termId` 是后加字段，存量行没有它，而按学期替换的逻辑遍布排课模块。本轮系统排查该模式的全部影响面。
- **审计方法**：① 对比真实库与新种子的字段差异；② 检索全部 20 处「按字段过滤删除」的写逻辑；③ 构造"升级前的老库"（抹掉全部按学期集合的 `termId`）逐一驱动写操作，检查重复主键与数据残留。
- **审计结论**：
  - 受影响集合共 10 个（班级、教室、教室资源覆盖、作息模板、课程规则、排课约束、任课配置、教师时间规则、排课草稿、调课申请），此前只补齐了班级与教室两个，其余仍是隐患——已将补齐范围扩展为常量 `TERM_SCOPED_COLLECTIONS` 统一覆盖。
  - 逐一验证各写操作：保存班级结构、课程规则、教室资源、任课配置在存量数据下均正常，无重复主键、无脏引用残留（课程规则确认为覆盖语义而非累加）。
  - 真实库现状核验：按学期作用域集合共 361 行，`termId` 完整率 100%。
- **测试加固**：新增用例覆盖全部按学期集合的补齐完整性，并改为**调用产品代码的 `normalizeDatabase`** 而非在测试内另写一份补齐逻辑——否则产品侧补齐范围遗漏时测试也发现不了。已通过"故意缩小产品侧补齐范围 → 测试失败"反向验证其有效性。
